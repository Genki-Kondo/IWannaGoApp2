...
import Firebase
import ViewAnimator
class LifeTableViewController: UIViewController,UITableViewDataSource,UITableViewDelegate,CLLocationManagerDelegate {
    //tableviewCellを１度リセットするための判定フラグ
    var deleteFrag = true
    //タップされたか判定するためのフラグ
    var tapFrag:Bool = false
    //情報を次々に入れていくための配列
    var lifeInfoArray = [Info]()
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if deleteFrag == true{
            ...
            cell.shopNameLabel.text = lifeInfoArray[indexPath.row].shopName
            cell.addressLabel.text = lifeInfoArray[indexPath.row].address
            cell.telNumberLabel.text = lifeInfoArray[indexPath.row].telNumber
            cell.StationLabel.text = lifeInfoArray[indexPath.row].station+"駅"
            cell.timeLabel.text = lifeInfoArray[indexPath.row].time+"分"
        }
    ...
    func tableView(_ tableView: UITableView, didHighlightRowAt indexPath: IndexPath) {
        if tapFrag == true{
            ...
            let likedShopName = lifeInfoArray[indexPath.row].shopName
            let likedAddress = lifeInfoArray[indexPath.row].address
            let likedTelNumber = lifeInfoArray[indexPath.row].telNumber
            let likedStation = lifeInfoArray[indexPath.row].station
            let likedTime = lifeInfoArray[indexPath.row].time
    ...
    func delete(){
        ...
        //データを削除
        DBRef.child("LifeInfo").removeValue()
    }
    //YOLPのAPIに送信し地図を表示する
    //そしてfirebaseにデータを送信する
    func getApiAndSendToDB(){
        let urlString = "https://map.yahooapis.jp/search/local/V1/localSearch?"+"&lat="+latitudeNow+"&lon=" + longitudeNow+"&gc="+selectedLifeDetail+"&maxprice=" + maxMoney+"&sort="+selectedShowType+"&parking="+parking+"&results=30&appid=dj00aiZpPVBHOVBCb0daWTZXRSZzPWNvbnN1bWVyc2VjcmV0Jng9Y2M-&output=json"
        AF.request(urlString).responseJSON { response in
            ...
            if let jsonObject = response.value{
                ...
                for (_,subJson):(String, JSON) in features{
                    ...
                    //firebase
                    let infoDB = Database.database().reference().child("LifeInfo").childByAutoId()
    ...
    func fetchContentsData (){
        let ref = Database.database().reference().child("LifeInfo").observe(.value) { (snapShot) in
            self.lifeInfoArray.removeAll()
            if let snapShot = snapShot.children.allObjects as? [DataSnapshot]{
                for snap in snapShot{
                    if let postData = snap.value as? [String:Any]{
                        ...
                        self.lifeInfoArray.append(Info(shopName: shopName!, address: address!, telNumber: telNumber!, station: station!, time: time!))
                    }
                }
                self.tableView.reloadData()
                let indexPath = IndexPath(row:self.lifeInfoArray.count - 1,section:0)
                if self.lifeInfoArray.count >= 5 {

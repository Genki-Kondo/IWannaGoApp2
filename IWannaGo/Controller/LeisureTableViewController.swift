...
import Firebase
import ViewAnimator
class LeisureTableViewController: UIViewController,UITableViewDataSource,UITableViewDelegate,CLLocationManagerDelegate {
    //tableviewCellを１度リセットするための判定フラグ
    var deleteFrag = true
    //タップされたか判定するためのフラグ
    var tapFrag:Bool = false
    //情報を次々に入れていくための配列
    var leisureInfoArray = [Info]()
    ...
    var maxMoney: String = ""
    //業種
    var selectedLeisureDetail: String = ""
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if deleteFrag == true{
            ...
            cell.shopNameLabel.text = leisureInfoArray[indexPath.row].shopName
            cell.addressLabel.text = leisureInfoArray[indexPath.row].address
            cell.telNumberLabel.text = leisureInfoArray[indexPath.row].telNumber
            cell.StationLabel.text = leisureInfoArray[indexPath.row].station+"駅"
            cell.timeLabel.text = leisureInfoArray[indexPath.row].time+"分"
        }
    ...
    func tableView(_ tableView: UITableView, didHighlightRowAt indexPath: IndexPath) {
        if tapFrag == true{
            ...
            let likedShopName = leisureInfoArray[indexPath.row].shopName
            let likedAddress = leisureInfoArray[indexPath.row].address
            let likedTelNumber = leisureInfoArray[indexPath.row].telNumber
            let likedStation = leisureInfoArray[indexPath.row].station
            let likedTime = leisureInfoArray[indexPath.row].time
    ...
    func delete(){
        ...
        //データを削除
        DBRef.child("LeisureInfo").removeValue()
    }
    //YOLPのAPIに送信し地図を表示する
    //そしてfirebaseにデータを送信する
    func getApiAndSendToDB(){
        let urlString = "https://map.yahooapis.jp/search/local/V1/localSearch?"+"&lat="+latitudeNow+"&lon=" + longitudeNow+"&gc="+selectedLeisureDetail+"&maxprice=" + maxMoney+"&sort="+selectedShowType+"&parking="+parking+"&results=30&appid=dj00aiZpPVBHOVBCb0daWTZXRSZzPWNvbnN1bWVyc2VjcmV0Jng9Y2M-&output=json"
        AF.request(urlString).responseJSON { response in
            ...
            if let jsonObject = response.value{
                ...
                for (_,subJson):(String, JSON) in features{
                    ...
                    //firebase
                    let infoDB = Database.database().reference().child("LeisureInfo").childByAutoId()
    ...
    func fetchContentsData (){
        let ref = Database.database().reference().child("LeisureInfo").observe(.value) { (snapShot) in
            self.leisureInfoArray.removeAll()
            if let snapShot = snapShot.children.allObjects as? [DataSnapshot]{
                for snap in snapShot{
                    if let postData = snap.value as? [String:Any]{
                        ...
                        self.leisureInfoArray.append(Info(shopName: shopName!, address: address!, telNumber: telNumber!, station: station!, time: time!))
                    }
                }
                self.tableView.reloadData()
                let indexPath = IndexPath(row:self.leisureInfoArray.count - 1,section:0)
                if self.leisureInfoArray.count >= 5 {

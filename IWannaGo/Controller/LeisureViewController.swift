...
class LeisureViewController: UIViewController,CLLocationManagerDelegate {
    ...
    func showMap(){
        ...
        //現在位置のアイコンを変更
        nowMarker.icon = self.imageWithImage(image: UIImage(named: "wolf")!, scaledToSize: CGSize(width: 50.0, height: 50.0))
        //変更を可能にする
        ...
        //YOLPのAPIを送信し地図を表示する
        let urlString = "https://map.yahooapis.jp/search/local/V1/localSearch?"+"&lat="+latitudeNow+"&lon=" + longitudeNow+"&gc="+selectedLeisureDetail+randomLeisureGc+"&maxprice=" + maxMoney + "&sort="+selectedShowType + "&parking="+parking+"&results=30&appid=dj00aiZpPVBHOVBCb0daWTZXRSZzPWNvbnN1bWVyc2VjcmV0Jng9Y2M-&output=json"
